...
async function detectHeadphones(imagePath) {
    ...
    console.log("Raw predictions length:", predictions.length);
    console.log("First 6 raw predictions:", predictions.slice(0, 6));
    ...
    for (let i = 0; i < predictions.length / 6; i++) {
        const baseIdx = i * 6;
        const cx = predictions[baseIdx];
        const cy = predictions[baseIdx + 1];
        const w = predictions[baseIdx + 2];
        const h = predictions[baseIdx + 3];
        const confidence = predictions[baseIdx + 4]; 
        const _class = predictions[baseIdx + 5];

        console.log(`Prediction ${i}:`, {
            cx, cy, w, h, confidence, class: _class
        });

       
        if (i < 5) {
            console.log(`Raw prediction ${i}: cx: ${cx}, cy: ${cy}, w: ${w}, h: ${h}, confidence: ${confidence}`);
        }
